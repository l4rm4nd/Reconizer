...
parser.add_argument("--hostfile", metavar='<file>', help="A txt file with ips per newline", type=str, required=True)
...
def prscan(hostfile):  
    ...
    with open(hostfile,'r') as fin:
        lines = fin.readlines()
    for line in lines:
        ips.append(str(line).rstrip("\n"))

    # get unique ips by converting to set and back
    ips = list(set(ips))
    ips = ips.copy()
